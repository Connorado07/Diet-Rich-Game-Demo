...
public class headTrack : OnAnimatorIKComponent
{
	...
	public override void updateOnAnimatorIKState ()
	{
		...
		if (lookInOppositeDirectionOutOfRange) {
			currentHorizontalAngle = Vector3.SignedAngle (transformForward, playerCameraManager.getCameraTransform ().forward, transformUp);
	...
	{ 
		if (playerControllerManager.canHeadTrackBeUsed () && !playerControllerManager.driving) {
			...
			if ((!playerIsAiming && IKManager.getHeadWeight () == 0) ||
			    ...
			    (!playerControllerManager.hasToLookInCameraDirectionOnFreeFire () && !isCheckToKeepWeaponAfterAimingWeaponFromShooting) ||
			
			    (playerIsAiming && IKManager.getHeadWeight () == 0 && isCheckToKeepWeaponAfterAimingWeaponFromShooting) ||

			    (playerIsAiming && headTrackActiveWhileAiming)) {
