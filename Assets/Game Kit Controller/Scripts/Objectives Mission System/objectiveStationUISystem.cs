...
public class objectiveStationUISystem : MonoBehaviour
{
		...
		if (useCharacterObjectivePanel) {
			...
			if (currentCharacterObjectiveIndex < currentObjectiveInfoList.Count) {
				...
				if (showMissionInfo) {
					...
					updateChracterObjectiveTextContent ("NO MISSIONS AVAILABLE", "", "", "");
				}
			} else {
			...
			for (int i = 0; i < currentObjectiveInfoList.Count; i++) {
				...
				if (objectiveCanBeAdded) {
					...
					currentobjectiveMenuIconElement.objectiveNameText.text = currentObjectiveInfo.mainObjectiveEventSystem.generalObjectiveName;
					currentobjectiveMenuIconElement.objectiveLocationText.text = currentObjectiveInfo.mainObjectiveEventSystem.objectiveLocaltion;

					if (mainObjectiveLogSystem.checkMinLevelOnMissions) {
						if (currentObjectiveInfo.mainObjectiveEventSystem.useMinPlayerLevel && showMissionsWithHigherLevel) {
							currentobjectiveMenuIconElement.objectiveMinLevelText.gameObject.SetActive (true);
							currentobjectiveMenuIconElement.objectiveMinLevelText.text = "Level: " + currentObjectiveInfo.mainObjectiveEventSystem.minPlayerLevel;
						}
					}
	...
	public void enableAndSetMissionCompletePanelInfo ()
	{
		...
		if (currentObjectiveEventSystem != null) {
			missionCompleteNameText.text = currentObjectiveEventSystem.generalObjectiveName;

			missionCompleteRewardsText.text = currentObjectiveEventSystem.objectiveRewards;
		}
	}
