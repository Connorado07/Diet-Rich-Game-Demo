...
//		EmeraldAISystem currentEmeraldAISystem = objectToDamage.GetComponent<EmeraldAISystem> ();
//
//		if (currentEmeraldAISystem != null) {
//			currentEmeraldAISystem.Damage ((int)damageAmount, EmeraldAISystem.TargetType.Player);
//		}
	}
...
//		}
	...
	public static void setStamina (float staminaAmount, GameObject objectToAddAmount)
	{
...
//									print (currentRagdollActivator.name);
	...
	public static void sendNoiseSignal (float noiseRadius, Vector3 noisePosition, LayerMask noiseLyaer, float noiseDecibels, 
	                                    bool forceNoiseDetection, bool showNoiseDetectionGizmo)
	{
		...
		if (colliders.Length > 0) {
			for (int i = 0; i < colliders.Length; i++) {
				...
				if (currentFindObjectivesSystem != null) {
					currentFindObjectivesSystem.checkNoisePosition (noisePosition, noiseDecibels, forceNoiseDetection);
				}
			}
		}
	}
	...
	public static bool giveInventoryObjectToCharacter (GameObject character, string inventoryObjectName, int inventoryObjectAmount, 
	                                                   Transform positionToInstantiateInventoryObjectPickup, float forceAmount, 
	                                                   float maxRadiusToInstantiate, ForceMode inventoryObjectForceMode, float forceRadius)
	{
		if (character == null) {
	...
	public static void checkIfPlayerIsLookingAtDeadTarget (GameObject currentPlayer, Transform deadTarget, Transform placeToShoot)
	{
		...
		if (mainPlayerComponentsManager != null) {
			playerCamera currentPlayerCamera = mainPlayerComponentsManager.getPlayerCamera ();
			...
			if (currentPlayerCamera.isPlayerLookingAtTarget ()) {
				...
				if (!isSameObject) {
					if (checkIfWeakSpotListContainsTransform (deadTarget.gameObject, currentTargetToLook)) {
						isSameObject = true;
					}
				}
