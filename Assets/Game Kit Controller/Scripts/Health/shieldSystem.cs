...
public class shieldSystem : MonoBehaviour
{
	...
	public void updateSliderState ()
	{
		...
		dead = mainHealth.isDead ();
		lastDamageTime = mainHealth.getLastDamageTime ();

		if (!dead) {
			if (regenerateShield) {
				if (constantShieldRegenerate) {
					if (regenerateShieldSpeed > 0 && shieldAmount < maxShieldAmount) {
						if (Time.time > lastShieldDamageTime + regenerateShieldTime && Time.time > lastDamageTime + regenerateShieldTime) {
