		...
		if (list.isExpanded) {
			...
			for (int i = 0; i < list.arraySize; i++) {
				...
				if (i < list.arraySize && i >= 0) {
					...
					if (list.GetArrayElementAtIndex (i).isExpanded) {
						expanded = true;
						showCombatTypeInfoListElement (list.GetArrayElementAtIndex (i));
					}
					EditorGUILayout.Space ();
		...
		if (list.isExpanded) {
			...
			for (int i = 0; i < list.arraySize; i++) {
				...
				if (i < list.arraySize && i >= 0) {
					...
					if (list.GetArrayElementAtIndex (i).isExpanded) {
						expanded = true;
						showCombatAttackInfoListElement (list.GetArrayElementAtIndex (i));
					}
					EditorGUILayout.Space ();
		...
		if (list.isExpanded) {
			...
			for (int i = 0; i < list.arraySize; i++) {
				...
				if (i < list.arraySize && i >= 0) {
					...
					if (list.GetArrayElementAtIndex (i).isExpanded) {
						expanded = true;
						showDamageTriggerActiveInfoListElement (list.GetArrayElementAtIndex (i));
					}
					EditorGUILayout.Space ();
...
//			}
		...
		if (list.isExpanded) {
			...
			for (int i = 0; i < list.arraySize; i++) {
				...
				if (i < list.arraySize && i >= 0) {
					...
					if (list.GetArrayElementAtIndex (i).isExpanded) {
						expanded = true;
						showSurfaceInfoOnMeleeAttackListElement (list.GetArrayElementAtIndex (i));
					}
					EditorGUILayout.Space ();
	...
	void showSurfaceInfoOnMeleeAttackListElement (SerializedProperty list)
	{
		...
		showSimpleList (list.FindPropertyRelative ("soundsList"));
