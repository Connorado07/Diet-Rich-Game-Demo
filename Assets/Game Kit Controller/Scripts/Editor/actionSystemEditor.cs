...
//		EditorGUILayout.PropertyField (list.FindPropertyRelative ("addRigidobyVelocityFromParent"));
		GUILayout.EndVertical ();
		...
		if (list.FindPropertyRelative ("useEventInfoList").boolValue) {
			...
			showEventInfoList (list.FindPropertyRelative ("eventInfoList"), actionInfoIndex);
			GUILayout.EndVertical ();
		}
	...
	void showEventInfoList (SerializedProperty list, int actionInfoIndex)
	{
		GUILayout.BeginVertical ();
		EditorGUILayout.PropertyField (list, false);
		if (list.isExpanded) {
