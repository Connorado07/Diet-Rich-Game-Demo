...
public class vehicleCreatorEditor : EditorWindow
{
	...
	[MenuItem ("Game Kit Controller/Create New Vehicle", false, 2)]
...
//		Debug.Log (rectSize);
	...
	void OnGUI ()
	{
		...
		currentVehicleName = (string)EditorGUILayout.TextField ("Vehicle Name", currentVehicleName); 
		...
		currentVehicle = EditorGUILayout.ObjectField ("Current Vehicle", currentVehicle, typeof(GameObject), true, GUILayout.ExpandWidth (true)) as GameObject;

		GUILayout.BeginHorizontal ();
		showGizmo = (bool)EditorGUILayout.Toggle ("Show Gizmo", showGizmo);
		useHandle = (bool)EditorGUILayout.Toggle ("Show Handle Tool", useHandle);
			...
			for (int i = 0; i < currentVehicleBuilder.vehiclePartInfoList.Count; i++) { 
				...
				if (currentVehiclePartInfo.expandElement) {
					...
					currentVehiclePartInfo.newVehicleMesh = 
						EditorGUILayout.ObjectField ("New Vehicles Mesh", currentVehiclePartInfo.newVehicleMesh, typeof(GameObject), true, GUILayout.ExpandWidth (true)) as GameObject;
					
					if (currentVehiclePartInfo.newVehicleMesh == null) {
						
						currentVehiclePartInfo.useVehiclePart = (bool)EditorGUILayout.Toggle ("Use Vehicle Part", currentVehiclePartInfo.useVehiclePart);
					
						currentVehiclePartInfo.showHandleTool = (bool)EditorGUILayout.Toggle ("Show Handle", currentVehiclePartInfo.showHandleTool);
					} else {
				...
				if (currentVehiclePartInfo.useVehiclePart) {
					if (currentVehiclePartInfo.newVehicleMesh != null) {
						...
						if (currentVehiclePartInfo.newVehicleMesh != currentVehiclePartInfo.temporalNewVehicleMesh) {
							...
							if (currentVehiclePartInfo.temporalVehicleMesh) {
...
//			GUILayout.FlexibleSpace ();
			...
			for (int i = 0; i < currentVehicleBuilder.vehicleSettingsInfoList.Count; i++) { 
				...
				if (currentVehicleSettingsInfo.useBoolState) {
					currentVehicleSettingsInfo.boolState = (bool)EditorGUILayout.Toggle (currentVehicleSettingsInfo.Name, currentVehicleSettingsInfo.boolState);
				}

				if (currentVehicleSettingsInfo.useFloatValue) {
					currentVehicleSettingsInfo.floatValue = (float)EditorGUILayout.FloatField (currentVehicleSettingsInfo.Name, currentVehicleSettingsInfo.floatValue);
				}
...
//			GUILayout.FlexibleSpace ();
			...
			"in the vehicle controller gameObject it self, to update the vehicle state.", 11, MessageType.Info);
	...
	public void createVehicleGameObject (bool instantiateGameObject)
	{
		...
		for (int i = 0; i < currentVehicleBuilder.vehiclePartInfoList.Count; i++) { 
			if (!newVehiclesMeshesAdded) {
				...
				if (currentVehiclePartInfo.newVehicleMesh) {
