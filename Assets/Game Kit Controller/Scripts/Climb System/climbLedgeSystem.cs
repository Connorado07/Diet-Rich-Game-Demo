		...
		if (climbLedgeActive && climbingLedge) {
			...
			if (!firstPersonActive) {
				if (Time.time < lastTimClimbLedge + timeToClimbLedgeThirdPerson) {
					...
					MatchTarget (climbLedgZone.position + playerTransform.forward * handOffset, climbLedgZone.rotation, AvatarTarget.RightHand,
...
//		}
	...
	{
		holdLedgeZone.position += playerTransform.right * holdOnLedgeOffset.x +
		playerTransform.up * holdOnLedgeOffset.y +
		playerTransform.forward * holdOnLedgeOffset.z;

		Quaternion currentPlayerRotation = playerTransform.rotation;
		playerTargetRotation = Quaternion.LookRotation (-surfaceFoundNormal, playerTransform.up);

		Vector3 rotationEuler = playerTargetRotation.eulerAngles;
	
		rotationEuler = Vector3.Scale (rotationEuler, playerTransform.up);
		...
		while (!targetReached) {
			...
			playerTransform.rotation = Quaternion.Lerp (currentPlayerRotation, playerTargetRotation, rotateTimer);

			teleportTimer += Time.deltaTime;

			normalAngle = Vector3.Angle (playerTransform.up, surfaceFoundNormal);   
	...
	IEnumerator rotateTowardLedgeOnFirstPersonCoroutine ()
	{
		...
		Quaternion currentPlayerRotation = playerTransform.rotation;
		playerTargetRotation = Quaternion.LookRotation (-surfaceToHangNormal, playerTransform.up);
		...
		while (!targetReached) {
			...
			mainPlayerCamera.transform.rotation = Quaternion.Lerp (currentPlayerRotation, playerTargetRotation, rotateTimer);
