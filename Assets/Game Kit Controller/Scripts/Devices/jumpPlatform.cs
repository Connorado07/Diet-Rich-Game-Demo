...
public class jumpPlatform : MonoBehaviour
{
	...
	void OnTriggerEnter (Collider col)
	{
		...
		if ((col.gameObject.CompareTag ("Player") && useWithPlayer) || (col.gameObject.CompareTag ("friend") && useWithNPC) || (col.gameObject.CompareTag ("enemy") && useWithNPC)) {
		...
		//if any other rigidbody enters the trigger, then
			...
			if (objectToImpulse) {
				...
				if (objectToImpulse.CompareTag ("vehicle") && currentVehicleHUDManager && useWithVehicles) {
	...
	void OnTriggerExit (Collider col)
	{
		...
		if (objectToImpulseList.Contains (objectToImpulse)) {
			if (objectToImpulse.CompareTag ("Player")) {
				if (useKeyToJumpWithPlayer) {
					playerControllerManager.useJumpPlatformWithKeyButton (false, jumpForce);
				}
			} else if (objectToImpulse.CompareTag ("vehicle")) {
				if (useKeyToJumpWithVehicles) {
