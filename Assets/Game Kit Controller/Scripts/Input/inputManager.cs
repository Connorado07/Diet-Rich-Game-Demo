...
//					print ("pressing keyboard");
	...
	public bool getJoystickButtonFromMultiAxesList (int multiAxesIndex, int axesIndex, buttonType type, int controllerNumber, bool canBeUsedOnPausedGame)
	{
		...
		if ((canBeUsedOnPausedGame || !gameManagerPaused) && (!usingTouchControls || allowGamepadInTouchDevice)) {
			...
			if (multiAxesIndex >= numberOfMultiAxes) {
				print ("WARNING: The input manager is trying to access to an action which has changed or has been removed, use the Update Input List button on Player Input Manager.");

				return false;
			}

			if (multiAxesIndex < 0 || axesIndex < 0) {
				print ("WARNING: The input manager is trying to access to a multi axes which has changed or has been removed, use the Update Input List button on Player Input Manager.");
...
//				print (currentAxes.Name + " " + currentAxes.touchButtonPanelIndex + " " + currentAxes.touchButtonIndex);
	...
	public void getTouchButtonList ()
	{
		...
		List<touchPanelsInfo> touchPanelsInfoList = playerInputManagerList [0].getTouchPanelsList ();
	...
	{
		GKC_Utils.setActiveGameObjectInEditor (touchPanel);
	}
