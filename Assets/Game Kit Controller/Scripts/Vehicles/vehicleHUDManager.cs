...
public class vehicleHUDManager : healthManagement
{
	...
	void Update ()
	{
		...
		if (projectilesListContainsElements && Time.time > lastDamageTime + 3) {
	...
	{
//		print ("STATE: " + passengerOnDriverSeat + " " + IKDrivingManager.getPassengerGameObjectList ().Count);
		...
		if (damageParticles != null) {
			...
			for (int i = 0; i < fireParticles.Count; i++) {
				...
				if (activate) {
					...
					if (fireParticles [i].gameObject != true) {
	...
	{
		IKDrivingManager.gameObject.GetComponent<inputActionManager> ().setInputManagerOnEditor ();
	}
