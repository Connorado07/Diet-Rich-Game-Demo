	...
	void FixedUpdate ()
	{
		...
		for (i = 0; i < wheelsListCount; i++) {
			...
			if (hitsAmount > 0) {
				...
				currentWheelMesh.position = raycastResults [0].point + currentWheelUp * (currentWheelRadius * localScaleY);
			...
			//the wheel is in the air
				...
				//set the wheel mesh position according to the values of the wheel collider
				currentWheelMesh.position = wheelCenterPosition - currentWheelUp * (currentWheelSuspensionDistance * localScaleY);
			}
		...
		for (i = 0; i < wheelsListCount; i++) {
			...
			if (currentWheel.steerable) {
				...
				if (grounded) {
					mainRigidbody.AddForceAtPosition (currentWheelCollider.transform.up * (mult * antiRollForceFront), currentWheelCollider.transform.position);  
				}
			}
		}
		...
		if (groundRear) {
			...
			if (jumpInpuPressed) {
				...
				mainRigidbody.AddForce (transform.up * (mainRigidbody.mass * vehicleControllerSettings.jumpPower));
		...
		if (!driving && !externalBrakeActive) {
		...
		for (i = 0; i < wheelsListCount; i++) {
			...
			if (currentWheel.wheelSlipAmountSideways > 0.25f || currentWheel.wheelSlipAmountForward > 0.5f) {
				Vector3 skidPoint = wheelGroundHit.point + mainRigidbody.velocity * (2 * Time.deltaTime);
