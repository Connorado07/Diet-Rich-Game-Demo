					...
					if (checkToLeaveSpaceToPartner) {
						...
						if (currentDistanceToTarget < minDistanceWithPartner) {
							...
							if (!leavingSpaceToPartner) {
								...
								Vector3 temporalPosition = partner.position + partner.right * (3 * randomDirection) + partner.forward * 1.5f;
								...
								if (currentNavMeshPath != null && currentNavMeshPathStatus == NavMeshPathStatus.PathComplete) {
									...
									temporalPosition = partner.position - partner.right * (3 * randomDirection) + partner.forward * 1.5f;
				...
				if (patrolling) {
					...
					if (runFromTarget) {
						...
						if (moveToTargetActive || leavingSpaceToPartner) {
							...
							currentTargetPosition = targetCurrentPosition + targetTransformUp * positionOffset + targetOffset * (minDistanceToMoveBack + 1);
