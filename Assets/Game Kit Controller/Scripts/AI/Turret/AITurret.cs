...
public class AITurret : MonoBehaviour
{
	...
	void Start ()
	{
		...
		laserBeam.GetComponent<enemyLaser> ().setOwner (turretAttacker);
	...
	void shootWeapon (GameObject enemyToShoot, Transform placeToShoot, bool checkTargetOnRaycast)
	{
		...
		if (timer >= shootTimerLimit) {
			...
			if (shellDropSoundList.Count > 0) {
				newClipToShell = shellDropSoundList [Random.Range (0, shellDropSoundList.Count - 1)];
			}
	...
	public void setDeathState ()
	{
		...
		damageInScreen currentDamageInScreen = GetComponent<damageInScreen> ();

		if (currentDamageInScreen != null) {
			Transform currentNumbersParent = currentDamageInScreen.numbersParent;

			if (currentNumbersParent != null) {
				currentNumbersParent.gameObject.SetActive (false);
			}
		}
