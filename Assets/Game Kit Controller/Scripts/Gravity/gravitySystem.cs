...
public class gravitySystem : gravityObjectManager
{
	...
	void OnCollisionEnter (Collision collision)
	{
		...
		if (searchingSurface && !playerControllerManager.isPlayerRunning () && turning && searchAround) {
			//check that the detected object is not a trigger or the player himself
			if (!collision.gameObject.CompareTag ("Player") && !collision.rigidbody && !collision.collider.isTrigger) {
