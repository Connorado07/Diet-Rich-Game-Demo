...
public class playerCamera : cameraControllerManager
{
	...
	{ 
		...
		if (mainCamera.enabled && mainGameManager != null) {
			...
			if (isCameraTypeFree ()) {
				if (cameraCanBeUsed) {
					if (!dead) {
						...
						if (pivotLeanTransform) {
							if (currentState.leanEnabled && leanActive) {
...
//					targetPosition = lockedCameraSpline.GetPoint (totalSplineProgress / 100);
	...
	public void checkCameraPosition ()
	{
		...
		if (cameraCollisionAlwaysActive || currentState.cameraCollisionActive) {
			if (Physics.SphereCast (pivotCameraPosition, maxCheckDist, directionFromPivotToCamera, out hit, distanceToCamPositionOffset + extraCameraCollisionDistance, settings.layer)) {
				...
				mainCameraTransform.localPosition = new Vector3 (cameraNewPosition.x, currentCamPositionOffset.y, cameraNewPosition.z);
			...
			if (fullTargetList [i] != null) {
				...
				if (tagToLookList.Contains (currentTarget.tag) || pointToLookComponentList.Contains (currentTarget)) {
					...
					if (objectVisible && checkObstaclesToTarget) {
						...
						float distance = GKC_Utils.distance (temporaltargetPosition, lookAtTargetTransform.position);
