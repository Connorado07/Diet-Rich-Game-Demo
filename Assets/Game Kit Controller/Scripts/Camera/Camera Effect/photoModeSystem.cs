...
public class photoModeSystem : MonoBehaviour
{
	...
	bool photoModeInputActive;

	void FixedUpdate ()
	{
		if (photoModeActive && photoModeInputActive) {
			movementAxisValues = playerInput.getPlayerMovementAxis ();
			...
			moveInput = (movementAxisValues.y * pivotTransform.forward + movementAxisValues.x * pivotTransform.right) * movementSpeed;

			if (movingCameraUp) {
				moveInput += Vector3.up * movementSpeed;
