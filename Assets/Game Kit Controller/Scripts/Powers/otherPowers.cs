...
public class otherPowers : MonoBehaviour
{
	...
	void Start ()
	{
		...
		if (settings.changePlayerMaterialsOnWallRun && playerRenderer != null) {
							...
							if (currentPower.tagToLocate.Contains (target.tag)) {
								...
								if (!locatedEnemies.Contains (placeToShoot)) {
									...
									playerScreenObjectivesManager.addElementToPlayerList (placeToShoot, false, false, 0, true, false, 
										false, false, currentPower.locatedEnemyIconName, false, Color.white, true, -1, 0, false);
								}
							}
						}
					}
	...
	void updateWallWalkState ()
	{
		if (wallWalkActive && wallWalkEnabled) {
			...
			if (playerControllerManager.isWallRunningActive () || playerControllerManager.isExternalControlBehaviorForAirTypeActive ()) {
				stopWallWalk ();
			}
			...
			if (!surfaceInFrontDetected && !Physics.Raycast (transform.position + transform.up, -transform.up, out hit, 5, settings.layer)) {
	...
	{
		if (wallWalkEnabled) {
			if (!wallWalkActive && !playerControllerManager.isWallRunningActive () || playerControllerManager.isExternalControlBehaviorForAirTypeActive ()) {
				...
				if (lastTimeRunActivated > 0) {
					...
					if (playerControllerManager.getRunButtonPressedTimer () > 0.5f) {
						if (!resetingSurfaceRotationAfterWallWalkActive || !gravityManager.isCharacterRotatingToSurface ()) {
...
//							}
	...
	public void activateWallWalk ()
	{
		if (wallWalkEnabled) {
			...
			if (settings.changePlayerMaterialsOnWallRun) {
	...
	{
		if (shootsettings.selectedPowerIcon != null) {
	...
	public void setTrailsInfo (Transform[] trailsPositions)
	{
		trails.Clear ();
