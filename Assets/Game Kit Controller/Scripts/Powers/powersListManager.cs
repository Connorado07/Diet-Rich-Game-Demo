...
public class powersListManager : MonoBehaviour
{
	...
	public void editPowersSlots ()
	{
		...
		if ((canBeOpened () || editingPowers) && !selectingPower) {
			...
			pauseManager.enableOrDisablePlayerMenu (editingPowers, false);
	...
	public void selectPowersSlots ()
	{
		...
		if ((canBeOpened () || selectingPower) && !editingPowers) {
			...
			pauseManager.enableOrDisablePlayerMenu (selectingPower, false);
	...
	{
		if (!pauseManager.isGamePaused () && !playerControllerManager.isUsingDevice () && powerListManagerEnabled && powersManager.isPowersModeActive () && !playerControllerManager.isPlayerMenuActive ()) {
			return true;
		}
