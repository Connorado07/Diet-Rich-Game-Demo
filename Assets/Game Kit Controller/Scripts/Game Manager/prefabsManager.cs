...
public class prefabsManager : MonoBehaviour
{
	...
	public void placePrefabInScene (int typeIndex, int prefabIndex)
	{
		...
		string prefabName = prefabTypeInfoList [typeIndex].prefabInfoList [prefabIndex].Name;

		if (prefabToInstantiate != null) {
			positionToInstantiate += Vector3.up * prefabTypeInfoList [typeIndex].prefabInfoList [prefabIndex].positionToSpawnOffset;
	...
	{
		for (int j = 0; j < prefabTypeInfoList [prefabTypeIndex].prefabInfoList.Count; j++) {
			if (prefabTypeInfoList [prefabTypeIndex].prefabInfoList [j].prefabGameObject) {
