...
public class arrowProjectile : MonoBehaviour
{
	...
	void FixedUpdate ()
	{
		if (!mainProjectileSystem.projectileUsed) {
			if (showDebugPrint) {
				print ("searching");
			}
			...
			if (applyArrowForces) {
				if (!useGravityOnArrow && gravityForceDownAmount > 0) {
					mainRigidbody.AddForce (-Vector3.up * (gravityForceDownAmount * mainRigidbody.mass));
				}
				...
				if (rotateArrowWithGravity) {
					if (gravityForceRotationSpeed > 0) {
						...
						transform.rotation = Quaternion.Lerp (transform.rotation, targetRotation, Time.deltaTime * gravityForceRotationSpeed);
					}
				}
			}
		}
	...
	public void checkObjectDetected (GameObject objectToDamage)
	{
		...
		mainProjectileSystem.projectileUsed = true;
	...
	{
		gravityForceRotationSpeed *= newValue;

		gravityForceDownAmount *= newValue;
	}
