...
public class weaponListManager : MonoBehaviour
{
	...
	public void selectWeaponsSlots (bool state)
	{
		...
		if ((canBeOpened () || selectingWeapon)) {
			...
			pauseManager.enableOrDisablePlayerMenu (selectingWeapon, false);
	...
	{
		if (!pauseManager.isGamePaused () && !playerControllerManager.isUsingDevice () && weaponListManagerEnabled && weaponsManager.isWeaponsModeActive () && !playerControllerManager.isPlayerMenuActive ()) {
			return true;
		}
