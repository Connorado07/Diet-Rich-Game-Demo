...
public class playerWeaponsManager : MonoBehaviour
{
					...
					if (!usedByAI) {
						if (changeWeaponsWithNumberKeysActive && !reloadingWithAnimationActive && !playerInput.isUsingTouchControls ()) {
							if (!currentIKWeapon.isWeaponMoving () &&
							    !gameSystemManager.isGamePaused () &&
	...
	{
		if (!playerManager.isUsingDevice () && !carryingPhysicalObject && !editingWeaponAttachments && !playerManager.isUsingSubMenu () && !playerManager.isPlayerMenuActive ()) {
			return false;
		}
		...
		if (currentWeaponAmmoText != null) {
			if (usingDualWeapon || changingDualWeapon) {
				if (!currentRightWeaponSystem.weaponSettings.infiniteAmmo) {
					currentRightWeaponAmmoText.text = currentRightWeaponSystem.getWeaponClipSize () + "/" + currentRightWeaponSystem.weaponSettings.remainAmmo;
				} else {
					currentRightWeaponAmmoText.text = currentRightWeaponSystem.getWeaponClipSize () + "/" + "Inf";
				}

				if (!currentLeftWeaponSystem.weaponSettings.infiniteAmmo) {
					currentLeftWeaponAmmoText.text = currentLeftWeaponSystem.getWeaponClipSize () + "/" + currentLeftWeaponSystem.weaponSettings.remainAmmo;
				} else {
					currentLeftWeaponAmmoText.text = currentLeftWeaponSystem.getWeaponClipSize () + "/" + "Inf";
				}
			} else {
				if (currentWeaponSystem != null) {
					if (!currentWeaponSystem.weaponSettings.infiniteAmmo) {
						currentWeaponAmmoText.text = currentWeaponSystem.getWeaponClipSize () + "/" + currentWeaponSystem.weaponSettings.remainAmmo;
					} else {
						currentWeaponAmmoText.text = currentWeaponSystem.getWeaponClipSize () + "/" + "Inf";
					}
...
//					print ("setting current weapon to " + currentWeaponName);
	...
	public bool checkAndSetWeaponsAvailable ()
	{
		...
		for (int i = 0; i < weaponListCount; i++) {
			if ((weaponsList [i].isWeaponEnabled () && (!loadCurrentPlayerWeaponsFromSaveFile || !gameSystemManager.isLoadGameEnabled ()))
	...
	{
		GameObject firstWeapon = getIKWeaponSystem (firstWeaponName).gameObject;
		GameObject secondWeapon = getIKWeaponSystem (secondWeaponName).gameObject;

		for (int j = 0; j < weaponPocketList.Count; j++) {
			for (int k = 0; k < weaponPocketList [j].weaponOnPocketList.Count; k++) {
				if (weaponPocketList [j].weaponOnPocketList [k].weaponList.Contains (firstWeapon) && weaponPocketList [j].weaponOnPocketList [k].weaponList.Contains (secondWeapon)) {
					return true;
				}
			}
		}
	...
	public bool dropCurrentWeapon (IKWeaponSystem weaponToDrop, bool ignoreRagdollCollision, bool checkChangeToNextWeapon)
	{
		...
		if (storePickedWeaponsOnInventory) {
			playerInventoryManager.dropEquipByName (weaponToDrop.getWeaponSystemName (), 1, false);
		}
	...
	public bool dropCurrentDualWeapon (IKWeaponSystem weaponToDrop, bool ignoreRagdollCollision, bool checkChangeToNextWeapon)
	{
		...
		if (storePickedWeaponsOnInventory) {
			playerInventoryManager.dropEquipByName (currentRightIKWeapon.getWeaponSystemName (), 1, false);
			playerInventoryManager.dropEquipByName (currentLeftIkWeapon.getWeaponSystemName (), 1, false);
		}
	...
	public void launchWeaponRigidbodyWhenDropWeapon (IKWeaponSystem weaponToDrop, bool ignoreRagdollCollision)
	{
		...
		if (dropWeaponInventoryObjectsPickups || weaponPickupToInstantiate == null) {
...
//		setWeaponByName (singleWeaponName);
	...
	public void editWeaponAttachments ()
	{
		if ((!playerManager.isPlayerMenuActive () || editingWeaponAttachments) &&
		    !playerManager.isUsingDevice () &&
		    !gameSystemManager.isGamePaused () &&
		    (!usingDevicesManager.hasDeviceToUse () || editingWeaponAttachments)) {
	...
	public bool canUseCarriedWeaponsInput ()
	{
		if (canMove &&
		    ...
		    !playerManager.isUsingGenericModelActive ()) {
