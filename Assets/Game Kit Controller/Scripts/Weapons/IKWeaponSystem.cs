...
public class IKWeaponSystem : weaponObjectInfo
{
	...
	public string relativePathWeaponsPickups = "Assets/Game Kit Controller/Prefabs/Weapons/Weapons Pickups";
	public GameObject emtpyWeaponPrefab;

	public string relativePathWeaponsMesh = "Assets/Game Kit Controller/Prefabs/Inventory/Mesh/Weapons";
	public string relativePathWeaponsAmmoMesh = "Assets/Game Kit Controller/Prefabs/Inventory/Mesh/Ammo";
		...
		if (currentWeapon && (thirdPersonWeaponInfo.useMeleeAttack || firstPersonWeaponInfo.useMeleeAttack) &&
			...
			if (!firstPersonActive) {
				if (thirdPersonWeaponInfo.useActionSystemForMeleeAttack) {
					weaponsManager.activateCustomAction (thirdPersonWeaponInfo.meleeAttackActionName);
				}
	...
	IEnumerator weaponStartMeleeAttack (float movementSpeed, float delayEndMeleeAttack)
	{
		...
		if (useMeleeAnimation) {
			...
			mainWeaponAnimatorSystem.setMeleeAttackState (true);
	...
	public void checkMeleeAttackDamage ()
	{
		...
		if (useCapsuleRaycast) {
			Collider[] hits = Physics.OverlapBox (raycastPosition.position, Vector3.one * raycastDistance / 2, raycastPosition.rotation, layer);

			Collider detectedSurface = null;

			if (hits.Length > 0) {
				detectedSurface = hits [0];

				if (hits [0].gameObject == player) {
					if (hits.Length >= 2) {
						detectedSurface = hits [1];
					}
				}
			}

			if (detectedSurface != null) {
				surfaceDetected = Physics.Raycast (raycastPosition.position, detectedSurface.transform.position - raycastPosition.position, out hit, raycastDistance, layer);
			}
		} else {
			surfaceDetected = Physics.Raycast (raycastPosition.position, raycastPosition.forward, out hit, raycastDistance, layer);
		}
		...
		if (surfaceDetected) {
			...
			foreach (Collider col in colliders) {
				...
				applyDamage.checkHealth (gameObject, objectToDamage, damageAmount, raycastPosition.forward, hit.point, 
...
//		print (firstPersonWeaponInfo.useReloadMovement);
	...
	IEnumerator setWeaponMeshPositionRotationCoroutine (Transform referenceTransform)
	{
		...
		while (t < 1 && (weaponMeshTransform.localPosition != targetPosition || weaponMeshTransform.localRotation != targetRotation)) {
	...
	public bool pickupAttachment (string attachmentName)
	{
		if (mainWeaponAttachmentSystem != null) {
			if (isAttachmentAlreadyActiveOnWeapon (attachmentName)) {
				print (attachmentName + " attachmnt already active on weapon");
	...
	{
		GKC_Utils.createInventoryWeaponAmmo (getWeaponSystemName (), getWeaponSystemAmmoName (), weaponAmmoMesh, weaponAmmoIconTexture, inventoryAmmoCategoryName, ammoAmountPerPickup);
	}
	...
	public void createInventoryWeapon ()
	{
		...
		GKC_Utils.createInventoryWeapon (getWeaponSystemName (), inventoryWeaponCategoryName, weaponMesh, newWeaponIconTexture, relativePathWeaponsMesh, false);

		assignWeaponPrefab ();
	}

	public void createInventoryWeaponExternally (string newWeaponName, Texture newWeaponIconTexture)
	{
	...
	public void createInventoryWeaponAmmoExternally (string newWeaponAmmoName)
	{
