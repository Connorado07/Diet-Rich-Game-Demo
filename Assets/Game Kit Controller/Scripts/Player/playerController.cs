...
public class playerController : MonoBehaviour
{
	...
	//States variables
	[HideInInspector] public bool gravityPowerActive;
	...
	bool changeScriptStateAfterFinishActionState;
	...
	void FixedUpdate ()
	{
		...
		if (usingAnimator && !useFirstPersonPhysicsInThirdPersonActive) {
			...
			if (playerOnGround) {
				...
				noAnimatorCurrentForce = currentMoveInput * (noAnimatorSpeed * noAnimatorCurrentMovementSpeed);
				...
				if (!wallRunningActive && !gravityPowerActive && !pauseAllPlayerDownForces && !externalForceOnAirActive && !externalControlBehaviorForAirTypeActive) {
			...
			if (!onGroundChecked) {
				...
				if (!isPlayerDead () && canRagdollMove) {
					...
					if (fallDamageEnabled && !gravityPowerActive && !fallDamageCheckPaused) {
						currentFallSpeed = mainRigidbody.velocity.magnitude;
						...
						if (checkFallState) {
							...
							if (applyFallDamageEnabled) {
								if (Time.time > currentLastTimeFalling + maxTimeInAirBeforeGettingDamage) {
									...
									float damageValue = totalTimeOnAir * mainRigidbody.velocity.magnitude;
		...
		if (!gravityPowerActive) {
			if ((usingAnimator && !useFirstPersonPhysicsInThirdPersonActive) ||
				...
				if (useMaxFallSpeed && !playerOnGround) {
					...
					if (verticalSpeed < 0) {
						...
						float verticalSpeedClamped = Mathf.Clamp (verticalSpeed, -maxFallSpeed, 100);
	...
	public void checkObstacleToAvoidMovement ()
	{
		...
		checkObstacleRaycastPosition -= playerTransformRight * (obstacleDetectionRaycastDistanceRightAndLeft * 2);
	...
	void onAirVelocity ()
	{
		if (!gravityPowerActive && !usingJetpack && !flyModeActive && !characterRotatingToSurface && !swimModeActive && !externalControlBehaviorForAirTypeActive) {
			...
			if (zeroGravityModeOn || freeFloatingModeOn) {
				...
				if (movingVertically) {
					...
					if (canMoveVerticallyAndHorizontal) {
						secondaryMoveInput += playerCameraTransform.up * (verticalInput * currentVerticalSpeed);	
					} else {
				...
				if (usingAnimator && !useFirstPersonPhysicsInThirdPersonActive) {
					if (externalForceOnAirActive) {
						...
						airMove = currentMoveInput * airSpeed + playerTransform.InverseTransformDirection (currentVelocity).y * playerTransformUp;
			...
			if (!playerOnGround) {
				...
				if (!gravityForcePaused && !zeroGravityModeOn && !freeFloatingModeOn && !wallRunningActive && !pauseAllPlayerDownForces && !externalControlBehaviorForAirTypeActive) {
					...
					extraGravityForce = playerTransformUp * (gravityForce * gravityMultiplier) + playerTransformUp * -gravityForce;
	...
	void updateAnimator ()
	{
		...
		if (customCharacterControllerActive) {
			...
			if (overrideOnGroundAnimatorValueActive) {
				...
				//			print (onGroundAnimatorState);
			} else {
				onGroundAnimatorState = playerOnGround;
			}

//			print (playerOnGround + "  " + onGroundAnimatorState + " " + previousValueOnGroundAnimatorState);
	...
	public void setActionActiveState (bool state)
	{
		...
		if (!actionActive) {
			if (changeScriptStateAfterFinishActionState) {
				//				print ("disable can move state again");

				changeScriptState (false);

				changeScriptStateAfterFinishActionState = false;
			}
	...
	public void setChangeScriptStateAfterFinishActionState (bool state)
	{
		changeScriptStateAfterFinishActionState = state;
	}
...
//							groundDetectedBySphereCast = true;
...
//							Vector3 previousplayerMovementDirection = playerMovementDirection;

							playerMovementDirection.Normalize ();

//							print (previousplayerMovementDirection + "  " + playerMovementDirection);
...
//									&& adherenceHit.rigidbody == null
	...
	{
		return (!jetPackEquiped && !flyModeActive && canMove && !usedByAI && !playerNavMeshEnabled && !driving && !swimModeActive);
	}
	...
	{
		if ((getMoveInputDirection ().magnitude > 0 && !isGravityPowerActive () &&
		    (isPlayerOnGround () || wallRunningActive || externalControlBehaviorForAirTypeActive) && !gravityManager.isSearchingSurface ()) || running) {
	...
	{
		if (!isUsingDevice () && !isUsingSubMenu () && !isPlayerMenuActive ()) {
			return false;
		}
		...
		if (usingExternalCharacterMeshList) {
			if (externalCharacterMeshList.Count > 0) {
				for (int i = 0; i < externalCharacterMeshList.Count; i++) {
					if (externalCharacterMeshList [i] != null) {
			...
			if (extraCharacterMeshGameObject.Count > 0) {
				for (int i = 0; i < extraCharacterMeshGameObject.Count; i++) {
					if (extraCharacterMeshGameObject [i] != null) {
