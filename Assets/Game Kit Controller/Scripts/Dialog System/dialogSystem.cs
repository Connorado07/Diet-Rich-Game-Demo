			...
			if (playDialogWithoutPausingPlayerActions) {
				if (showDialogLineWordByWord || showDialogLineLetterByLetter) {
					...
					if (textingShowingPartByPart && showFullDialogLineOnInputIfTextPartByPart) {
						...
						simpleDialogText.text = currentDialogInfoList [currentDialogContentIndex].dialogOwnerName + ": " + currentDialogInfoList [currentDialogContentIndex].dialogContent;
					} else {
	...
	IEnumerator playNextDialogWithoutPausingPlayerShowingDialogLinePartByPartCoroutine ()
	{
		...
		if (showDialogLineWordByWord || showDialogLineLetterByLetter) {
			if (showDialogLineWordByWord) {
				simpleDialogText.text = currentDialogInfoList [currentDialogContentIndex].dialogOwnerName + ": ";

				string[] words = Regex.Split (currentDialogLine, @"(?<=[.,;])");
				//currentDialogLine.Split (' ');
			...
			if (showDialogLineLetterByLetter) {
				for (int j = 0; j < currentDialogLine.Length; j++) {
					simpleDialogText.text = currentDialogInfoList [currentDialogContentIndex].dialogOwnerName + ": " + currentDialogLine.Substring (0, j);
...
//		print ("dialog condition result is " + state);
	...
	public void updateDialogContent ()
	{
		...
		mainLineText.text = temporalDialogInfo.dialogContent;
		...
		if (playDialogWithoutPausingPlayerActions) {
			simpleDialogText.text = temporalDialogInfo.dialogOwnerName + ": " + temporalDialogInfo.dialogContent;
		}
		...
		if (currentDialogContentSystem.showDialogOnwerName) {
			currentDialogOwnerNameText.text = temporalDialogInfo.dialogOwnerName;
		}
		...
		if (currentLineHasOptions) {
			...
			for (int i = 0; i < currentOptionsAmount; i++) {
				...
				dialogPanelInfoList [i].dialogOptionText.text = temporalDialogInfo.dialogLineInfoList [i].dialogLineContent;
				dialogPanelInfoList [i].dialogOptionGameObject.SetActive (true);
				...
				if (temporalDialogInfo.dialogLineInfoList [i].useStatToShowLine) {
					...
					dialogPanelInfoList [i].dialogOptionText.text += temporalDialogInfo.dialogLineInfoList [i].extraDialogLineContent;
				} else {
		...
		if (temporalDialogInfo.useDialogLineSound && temporalDialogInfo.dialogLineSound != null) {
			dialogAudioSource.PlayOneShot (temporalDialogInfo.dialogLineSound);
		} 
