...
public class grapplingHookTargetsSystem : MonoBehaviour
{
	...
	void LateUpdate ()
	{
		if (!showGrapplingHookTargetsActive || showGrapplingHookTargetsPaused || grapplingHookTargetInfoList.Count == 0) {
			return;
		}
		...
		if (anyTargetVisible) {
			...
			if (closestTargetInfo != null && closestTargetInfo.targetRectTransform) {
	...
	public void enableOrDisableGrapplingHookTargets (bool state)
	{
		for (int i = 0; i < grapplingHookTargetInfoList.Count; i++) {
			...
			if (currentGrapplingHookTargetInfo.targetTransform) {
