...
//				print (child.name);
	...
	public void calculateThrowDirection ()
	{
		...
		if (Physics.Raycast (raycastOrigin, raycastDirection, out hit, maxRaycastDistance, layerToCheckThrowObject)) {
			...
			if (useMaxDistanceWhenNoSurfaceFound) {
				...
				if (Physics.Raycast (targetPositionTransform.position, -targetPositionTransform.up, out hit, Mathf.Infinity, layerToCheckThrowObject)) {
					currentTargetPosition = hit.point;
				}
			}
		}
