...
public class closeCombatSystem : MonoBehaviour
{
	...
	bool canMoveStatePaused;

	bool rootMotionStateChanged;
	...
	public void useAttack (string attackName)
	{
		...
		for (int i = 0; i < combatTypeInfoListCount; i++) {
			...
			if (combatTypeID == currentCombatTypeInfo.combatTypeID) {
				...
				if (currentCombatTypeInfo.Name.Equals (attackName)) {
					...
					if (currentCombatTypeInfo.currentAttackIndex < currentCombatTypeInfo.combatAttackInfoList.Count) {
						...
						if (currentCombatTypeInfo.attackActive) {
							...
							if (Time.time > lastTimeComboAttack + minTimeToPlayNextAttack) {
								if (!currentCombatAttackInfo.playingNextAttack) {
									...
									if (currentCombatTypeInfo.comboCounter < currentCombatTypeInfo.combatAttackInfoList.Count) {
										...
										if (!nextAttackInfo.canMoveWhileAttackActive) {
											setMoveInputPausedState ();
										}
							...
							if (!currentCombatAttackInfo.canMoveWhileAttackActive) {
								setMoveInputPausedState ();
							}
						...
						if ((currentCombatTypeInfo.currentAttackIndex - 1) > currentCombatTypeInfo.comboCounter || currentCombatAttackInfo.playingNextAttack) {
							if (currentCombatTypeInfo.comboCounter <= j) {
								...
								if (currentCombatTypeInfo.comboCounter < currentCombatTypeInfo.combatAttackInfoList.Count) {
									...
									if (!attackToUse.canMoveWhileAttackActive) {
										setMoveInputPausedState ();
									}
	...
	public void setDamageDetectedOnTriggerById (int newId)
	{
		...
		checkEventOnDamage (currentcombatLimbInfo.hitCombatManager.currentCombatTypeIndex, currentcombatLimbInfo.hitCombatManager.currentAttackIndex, 
			currentcombatLimbInfo.hitCombatManager.getLastSurfaceDetected (), newId);
	}
	...
	bool canUseCombat ()
	{
		if (currentPlayerMode) { 
			if (combatSystemEnabled &&
			    !playerControllerManager.isGravityPowerActive () &&
			    playerControllerManager.isPlayerOnGround () &&
			    !playerControllerManager.isPlayerUsingPowers () &&
			    !playerControllerManager.isPlayerUsingWeapons () &&
			    !playerControllerManager.isUsingDevice () &&
			    !playerControllerManager.isPlayerDead () &&
			    !playerControllerManager.isPlayerMenuActive () &&
			    !playerControllerManager.isPlayerNavMeshEnabled ()) {
	...
	void disableCombat ()
	{
		...
		if (canMoveStatePaused) {
			playerControllerManager.setMoveInputPausedState (false);

			canMoveStatePaused = false;
		}
	...
	void setMoveInputPausedState ()
	{
		playerControllerManager.setMoveInputPausedState (true);
		playerControllerManager.resetPlayerControllerInput ();

		canMoveStatePaused = true;
	}
		...
		if (state) {
			...
			for (int i = 0; i < combatLimbList.Count; i++) {
				...
				if (state) {
					...
					if (place == colliderPlace.in_front && i == 0) {
						canUseLimb = true;
					}
...
//					print (currentSurfaceInfo.surfaceName);
	...
	public void udpateHitCombatInfo ()
	{
		for (int i = 0; i < combatLimbList.Count; i++) {
			...
			hitCombatToCheck.addForceMultiplier = addForceMultiplier;

			hitCombatToCheck.setTriggerId (i);
