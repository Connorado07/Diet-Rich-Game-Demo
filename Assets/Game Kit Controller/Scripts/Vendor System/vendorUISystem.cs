	...
	public void createCategorySlots ()
	{
		...
		for (int i = 0; i < mainInventoryManager.inventoryCategoryInfoList.Count; i++) {
			...
			if (objectsAvailableForCurrentCategory > 0) {
				...
				currentVendorCategorySlotPanelInfo.categoryNameText.text = currentVendorCategorySlotPanelInfo.Name;
...
//		LayoutRebuilder.ForceRebuildLayoutImmediate (vendorObjectListScrollbar.GetComponent<RectTransform> ());
	...
	public void createVendorObjectIcon (inventoryInfo currentInventoryInfo, int index)
	{
		...
		currentVendorObjectSlotPanelInfo.objectNameText.text = currentInventoryInfo.Name;
		...
		if (buyingObjects) {
			currentVendorObjectSlotPanelInfo.objectPriceText.text = currentInventoryInfo.vendorPrice + currentObjectSelectedPriceExtraText;
		} else {
			currentVendorObjectSlotPanelInfo.objectPriceText.text = currentInventoryInfo.sellPrice + currentObjectSelectedPriceExtraText;
		}
	...
	public void getCategoryPressedButton (Button buttonObj)
	{
		...
		currentCategoryNameText.text = categoryName;
	...
	public void setObjectInfo (int index)
	{
		if (index > -1) {
			...
			currentObjectNameText.text = currentInventoryObject.Name;

			currentObjectInformationText.text = currentInventoryObject.objectInfo;
	...
	public void setNumberOfObjectsToUseText (int amount)
	{
		...
		if (currentInventoryObject != null) {
			if (buyingObjects) {
				currentObjectSelectedPriceText.text = currentInventoryObject.vendorPrice + currentObjectSelectedPriceExtraText;
			} else {
				currentObjectSelectedPriceText.text = currentInventoryObject.sellPrice + currentObjectSelectedPriceExtraText;
			}
			...
			if (buyingObjects) {
				totalPriceText.text += (amount * currentInventoryObject.vendorPrice) + currentObjectSelectedPriceExtraText;
			} else {
				totalPriceText.text += (amount * currentInventoryObject.sellPrice) + currentObjectSelectedPriceExtraText;
			}
		}
	}
	...
	public void confirmBuyObject ()
	{
		...
		if (enoughMoneyAvailable) {
			...
			if (currentInventoryObject.spawnObject || currentInventoryObject.cantBeStoredOnInventory) {
				...
				GameObject inventoryObjectPrefab = mainInventoryManager.getInventoryPrefab (currentInventoryObject.inventoryGameObject);

				inventoryListManager.spawnInventoryObject (inventoryObjectPrefab, positionToSpawnObjects, currentInventoryObject.amount, currentInventoryObject);
