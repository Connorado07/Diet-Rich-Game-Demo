...
public class grabObjects : MonoBehaviour
{
	...
	{
		if (currentPhysicalObjectToGrabFound && grabObjectsPhysicallyEnabled) {
...
//							mainCameraTransform.TransformDirection (Vector3.right));
	...
	{
		if (foundObjectToGrabCursor != null) {
	...
	{
		if (objectHeld == null || currentPhysicalObjectGrabbed) {
			dropObject ();
		}
	}
	...
	{
		if (powerSlider != null) {
			if (powerSlider.gameObject.activeSelf != false) {
				powerSlider.gameObject.SetActive (false);
			}
		}
...
//			objectToGrabTransform.localScale = Vector3.one;
	...
	IEnumerator setGrabbedObjectClonnedColliderEnabledStateCoroutine ()
	{
		if (grabbedObjectClonnedCollider != null) {
			...
			grabbedObjectClonnedCollider.enabled = true;
		}
	}
...
//						print ("using current bone to keep object " + currentBone.name);
//
//						print (positionToKeepObject.name);
...
//			currentPhysicalObjectGrabbed.transform.localScale = Vector3.one;
	...
	public void inputGrabObject ()
	{
		...
		if (!playerControllerManager.isUsingDevice () &&
		    (aiming || currentPhysicalObjectToGrabFound) &&
		    objectHeld == null &&
		    grabObjectsEnabled) {
