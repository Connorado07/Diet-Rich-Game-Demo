...
public class consoleMode : MonoBehaviour
{
	...
	public void checkPrefabsManager ()
	{
		if (mainPrefabsManager == null) {
			...
			if (mainPrefabsManager != null) {
				GameObject newPrefabsManager = (GameObject)Instantiate (prefabsManagerPrefab, Vector3.zero, Quaternion.identity);

				newPrefabsManager.name = "Prefabs Manager";

				mainPrefabsManager = newPrefabsManager.GetComponent<prefabsManager> ();
			}
		}
	}
	...
	public void killAllEnemies ()
	{
		...
		for (int i = 0; i < healthList.Length; i++) {
			if (!healthList [i].gameObject.CompareTag ("Player") && !healthList [i].gameObject.CompareTag ("friend")) {
